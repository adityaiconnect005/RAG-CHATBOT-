...
Facts-only
Retrieval gates what the model may say; prompts and post-checks forbid advice and comparisons.
Single canonical source per answer
Retrieval returns chunks tagged with one citation URL; generation is constrained to cite that URL only.
Curated corpus
Ingestion is batch or scheduled from an allowlist of URLs; no arbitrary web crawling at query time.
No PII
No user document upload path; chat payloads exclude identifiers; logs redact or omit sensitive fields.
Accuracy over “intelligence”
Prefer abstention, refusal, or “see the indexed scheme page” over speculative answers.


Components in brief:
Scheduler (GitHub Actions): A repository workflow that will run every day at 9:15 AM IST and get the latest data. It executes the full ingest job so the index reflects the latest scraped HTML from the allowlisted URLs. See §4.0 for workflow details, secrets, and manual workflow_dispatch.
Scraping service: A service to get the data from the URLs. On each triggered run, it reads the URL registry, fetches every allowlisted page (HTTP(S), rate-limited, identifiable user-agent), persists raw HTML for audit/replay, and passes content into normalize → chunk → embed → index. It does not crawl beyond the registry.
...
Allowlisted URLs (HTML only; no PDFs in this phase):

- **HDFC Mutual Funds (AMC Page)**: https://groww.in/mutual-funds/amc/hdfc-mutual-funds
...
- **HDFC Technology Fund Direct Growth**: https://groww.in/mutual-funds/hdfc-technology-fund-direct-growth
- **HDFC Value Fund Direct Plan Growth**: https://groww.in/mutual-funds/hdfc-value-fund-direct-plan-growth
- **HDFC Manufacturing Fund Direct Growth**: https://groww.in/mutual-funds/hdfc-manufacturing-fund-direct-growth
- **HDFC Transportation and Logistics Fund Direct Growth**: https://groww.in/mutual-funds/hdfc-transportation-and-logistics-fund-direct-growth
...
- **HDFC NIFTY100 Quality 30 Index Fund Direct Growth**: https://groww.in/mutual-funds/hdfc-nifty100-quality-30-index-fund-direct-growth
- **HDFC NIFTY100 Low Volatility 30 Index Fund Direct Growth**: https://groww.in/mutual-funds/hdfc-nifty100-low-volatility-30-index-fund-direct-growth
- **HDFC NIFTY200 Momentum 30 Index Fund Direct Growth**: https://groww.in/mutual-funds/hdfc-nifty200-momentum-30-index-fund-direct-growth
...
- **HDFC Money Market Fund Direct Growth**: https://groww.in/mutual-funds/hdfc-money-market-fund-direct-growth
- **HDFC Ultra Short Term Fund Direct Growth**: https://groww.in/mutual-funds/hdfc-ultra-short-term-fund-direct-growth
- **HDFC Low Duration Fund Direct Growth**: https://groww.in/mutual-funds/hdfc-low-duration-fund-direct-growth
- **HDFC Medium Term Debt Fund Direct Growth**: https://groww.in/mutual-funds/hdfc-medium-term-debt-fund-direct-growth
...
Purpose
source_url
Canonical URL for citation (exactly one per assistant message).
source_type
e.g., groww_scheme_page (current); later factsheet, kim, sid, amfi, sebi if the corpus expands
scheme_id / scheme_name
Tie chunks to a scheme when applicable; null for generic regulatory pages.
amc
AMC identifier or name.
title
Page or section title for debugging and UI tooltips.
fetched_at
ISO date for “Last updated from sources” (can be corpus max date or per-source).
content_hash
Detect content drift on re-crawl.
...
One record per scheme per scrape run (or latest snapshot) with typed or labeled fields
Exact answers, filters, “last value we saw”, easy regression tests
Vector index (chunks)
Full normalized text / tables from the same page
Narrative context, exit load, benchmark, objectives, anything not modeled as columns
...
Field
Storage notes
scheme_id, scheme_name, amc
From URL registry; stable keys.
source_url
Groww page URL (citation).
fetched_at
ISO timestamp of scrape.
raw_content_hash
Hash of raw HTML used for extraction (traceability).
nav
Number + currency (e.g. INR) + optional as_of date if present on page.
minimum_sip
Number (INR) + frequency if stated (e.g. monthly).
fund_size / aum
Number + unit (e.g. ₹ Cr or raw string + parsed numeric).
expense_ratio
Percentage as number (e.g. 0.52 for 0.52% p.a.) + label (Direct/Regular) if applicable.
rating
Store the raw label from the page (e.g. “5”, “4★”, “Very High risk”) and a rating_kind enum: riskometer | analyst | unknown — Groww pages mix risk wording with other ratings; do not collapse unlike concepts.
...
Product default: Will run every day at 9:15 AM IST (Asia/Kolkata) to get the latest data.
...
Workflow responsibilities: The scheduled job in .github/workflows/ingest.yml checks out the repo, installs dependencies, and runs in order: 4.0 scrape → 4.1 normalize → 4.2 chunk + embed (local BAAI/bge-small-en-v1.5) → 4.3 upsert to Chroma Cloud. See chunking-embedding-architecture.md for chunk/embed details.
Secrets: Embedding uses local BAAI/bge-small-en-v1.5 (no API key). Chroma: vectors live in a hosted Chroma Cloud instance. Phase 4.3 and the runtime retriever use the same CHROMA_HOST / CHROMA_API_KEY settings. The workflow requires these secrets to be set in the repository to authenticate the ingestion. Optional: INGEST_USER_AGENT for scraping via ${{ secrets.* }}. Never commit .env with production keys.
...
Behavior: Get the data from URLs. For each URL, perform an HTTP(S) GET (or the minimal requests needed if the site uses client-rendered data—then document whether headless fetch is required). Respect robots.txt, rate limits between requests, and use a stable User-Agent string identifying the assistant project.
...
Index (phase 4.3 — Chroma Cloud)
Upsert vectors and metadata into a hosted Chroma Cloud instance via HttpClient (see §4.3 below). Same embedding dimension as ingest (384 for bge-small-en-v1.5).
Refresh
...
4.3 Phase: Vector index — Chroma Cloud
Product choice: Use [Chroma](https://www.trychroma.com) for dense retrieval with chromadb.HttpClient connecting to a hosted Chroma Cloud instance. Ingest (phase 4.3) and the app’s retriever (§5) authenticate with the same host and API key.
Ingest-time steps (ordered)
Client & path
HttpClient(host=...) pointing at the CHROMA_HOST, authenticated via x-chroma-token headers.
Collection
One logical collection per deployment (e.g. mf_faq_chunks), or per environment (mf_faq_dev / mf_faq_prod), created with get_or_create_collection.
...
CI (GitHub Actions)
After phase 4.2: run phase 4.3 with the Chroma API credentials injected from repository secrets. The runner sends the chunks over the network to the cloud database.
Query-time (runtime API — future wiring)
Use the same HttpClient connection parameters as ingest.
...
User message → pipeline → assistant message.
GET /health
Liveness.
POST /admin/reindex
Protected re-ingestion trigger (optional).
...
Vector DB
Chroma Cloud — phase 4.3 (HttpClient pointing at hosted instance; same setup at query time in §5)
Embeddings
BAAI/bge-small-en-v1.5 via sentence-transformers (local, 384-dim, 512 max tokens); upgrade to bge-base-en-v1.5 (768-dim) when corpus grows
LLM
Groq chat API for phase 6 (GROQ_API_KEY; model e.g. llama-3.1-8b-instant) — independent of local BGE embeddings
Orchestration
LangChain/LlamaIndex or thin custom pipeline
UI
Next.js, Streamlit, or minimal static + API
Storage
Postgres for threads/messages; object store for raw fetches
...
Deliverable
Where it lives in this architecture
README setup, AMC/schemes, architecture, limitations
Operational docs + this file + chunking-embedding-architecture.md (incl. phase 4.3 Chroma steps) + runtime/phase_5_retrieval/ (§5) + runtime/phase_6_generation/ (§6) + runtime/phase_7_safety/ (§7) + runtime/phase_8_threads/ (§8) + runtime/phase_9_api/ (§9).
Disclaimer snippet
UI + optional system prompt reinforcement.
Multi-thread chat
Section 8 + thread API in Section 9.
Facts-only + one citation + footer
Sections 5–7 and 6.2.
